...
function EditUser({ setShowEditUser, currentUser }) {
  const classes = useStyles();
  const [accNumber, setAccNumber] = useState(currentUser.accNumber);
      ...
      if (accNumber.length < 1) {
        ...
        const response = await axios({
          ...
          params: {
            ...
            email,
            password,
            checkEmail: email != currentUser.email,
          },
        });
        setEmail("");
        setAccNumber("");
          ...
          <Grid container spacing={2} className={classes.submit}>
            <Grid item spacing={3}>
              <Button
                color="info"
                variant="contained"
                onClick={async () => setShowEditUser(false)}
              >
                Cancel
              </Button>
            </Grid>
